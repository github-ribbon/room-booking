...
<div id="main_container">
	<h1 id="page_title">
		<c:choose>
			<c:when test="${period.equals('previous')}">
				<spring:message code="word.previousRes" />
			</c:when>
			<c:when test="${period.equals('upcoming')}">
				<spring:message code="word.upcomingRes" />
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${status.equals('C')}">
				<spring:message code="word.waitingReservations" />
			</c:when>
			<c:when test="${status.equals('D')}">
				<spring:message code="word.cancelledReservations" />
			</c:when>
			<c:when test="${status.equals('U')}">
				<spring:message code="word.bookedReservations" />
			</c:when>
		</c:choose>
	</h1>
