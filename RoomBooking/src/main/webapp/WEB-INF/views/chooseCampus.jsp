...
<div id="main_container">
	<h1 id="page_title">
		<spring:message code="title.choosingCampus" />
	</h1>
	<c:choose>
		...
		<c:otherwise>
			<nav id="context_menu">
				<sec:authorize access="hasRole('ROLE_ADMIN')">
					<a href="<c:url value="/campus/new" />" target="_blank"><spring:message
							code="word.newCampus" /></a>
				</sec:authorize>
			</nav>
			<div class="content">
				<c:choose>
								...
								<c:forEach items="${campuses}" var="item">
									<tr>
										<td><c:choose>
												...
												<c:when
													test="${source.equals('building')&& role.equals('admin')}">
													<form action="<c:url value="/building/new" />"
														class="small" method="get">
												</c:when>
												<c:when
